...
class CreateDenoisedImageInputSpec(BaseInterfaceInputSpec):
    ...
    time_course_image = File(exists=True, mandatory=True,
                             desc='The fMRI time course as an image')
    ica_mask_image = File(exists=True, mandatory=True,
                          desc='The ICA mask image')
    coma_rest_lib_path = Directory(
        exists=True, mandatory=True, desc='The ICA mask image')
    repetition_time = traits.Float(
        mandatory=True, desc='The repetition time (TR) in seconds')
...
class CreateDenoisedImageOutputSpec(TraitedSpec):
    neuronal_image = File(
        exists=True, desc='Reconstructed "denoised" image from neuronal components')
    non_neuronal_image = File(
        exists=True, desc='Reconstructed "noise" image from non-neuronal components')
...
class CreateDenoisedImage(BaseInterface):
    ...
    def _list_outputs(self):
        ...
        outputs['non_neuronal_image'] = op.abspath(
            self.inputs.out_non_neuronal_image)
        return outputs
...
class ComputeFingerprintInputSpec(BaseInterfaceInputSpec):
    ...
    component_index = traits.Int(
        desc='Index of the independent component to use from the t-value threshold file.')
    out_stats_file = File(
        desc='Reconstructed "denoised" image from neuronal components')
